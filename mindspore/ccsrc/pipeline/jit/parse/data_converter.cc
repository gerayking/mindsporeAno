...
/*转化python的list为vector
 * 遍历pythonlist内的元素，并转换成相应的值节点，然后存入value_list中
 * 如果转化成Value失败，返回false，全部成功返回false
 * */
...
FuncGraphPtr ConvertToFuncGraph(const py::object &obj, const std::string &python_mod_get_parse_method) {
  ...
  /*这里是使用了unordered_map来存储 健值->对象 的模式，类似创建了一个缓冲区，实现内存复用
   * */
 /*  如果obj在缓冲区中，那么将其指针放到value中
 *  如果指针是指向的是函数图对象
 *  就返回恰当类型的指针
 *  */
...
/* 2020年8月23日 11:35:17
 * 不在缓冲区
 * 获取nn.Cell中的contruct方法或者_hook_construct方法
 * */
...
std::vector<std::string> GetObjKey(const py::object &obj) {
  /*2020年8月23日 00:05:45
   * 导入mindspore._extends.parse中的文件
   * */
  py::module mod = python_adapter::GetPyModule(PYTHON_MOD_PARSE_MODULE);
  /*获得对象的唯一签名
   * */
