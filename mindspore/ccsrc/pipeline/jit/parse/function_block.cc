...
/*类的成员有两种，一种是方法，另一种是变量，分别进行处理
 * 创建一个新的命名空间
 *{obj.__module__}..<{obj.__class__.__name__}::{id(obj)}>
 *
 * */
...
AnfNodePtr FunctionBlock::MakeResolve(const NameSpacePtr &name_space, const SymbolPtr &resolve_symbol) {
  ...
  // 添加一个新的节点命名空间为name_space, 值为symbol_node
...
/* @phi 参数，这里表示的是与其他function block 有交互的参数
 * 判断一个参数是不是可替代的，如果有前驱图中有可替代的节点，则不需要生成
 * 否则遇到生成一个新的param节点
 * */
...
bool FunctionBlock::CollectRemovablePhi(const ParameterPtr &phi) {
  ...
  /*
   * 如果发现当前的参数节点是非必须的，即前面已经出现过或者有着相同的值
   * 那么递归的便利前面的可移除节点，共同指向一个节点，此操作避免生成非必要的节点
   *
   * */
  if (arg_node != nullptr) {
...
/* 这里是查看前面已经转化完成的节点，都会存储再phi_node中
 * 如果已经转化完成，就直接
 *
 * */
