...
class Parser {
 ...
 private:
  ...
  void BuildMethodMap();
  // 这个函数将 功能函数包装成一个流水线仅有输入输出，也就是将parse包装起来了
...
/*此处弱引用是为了防止出现循环依赖，循环引用的现象，出现内存泄漏
 * */
  ...
  // Save current loops to support 'continue', 'break' statement.MakeFunctionBlock
  std::stack<Loop> loops_;
};
...
class ParseAst {
 ...
 private:
  ...
  // ast 抽象语法树
  py::object ast_tree_;
  // py端的Parser 即 用于将python代码进行转化的工具类
  py::object parser_;
  // 调用py端的工具类
  py::module module_;
