...
/*  判断对象是否有效，
 *  如果对象有效，将这个对象构造成抽象语法树
 *  调用python的代码，获取抽象语法树的信息
 * */
...
/* 用抽象语法树ast构造parser类
 * 然后使用，parser工具类将抽象语法树ast转化成FuncGraph
 * 并且返回FuncGraph
 * */
...
/* 这个函数负责控制精度，如果有混合精度的，必须统一精度进行运算
 *
 * */
...
/*初始化转化的环境你，其实也就是初始化该网络的作用于
 * */
void Parser::InitParserEnvironment(const py::object &obj) {
  //获得一个弱引用，但是这里似乎没有什么用？
  Parser::top_func_graph_ = FuncGraphWeakPtr();
  /*这里运用了单例的设计模式，GetInstance()中静态声明了一个scopeManager
   * 这样，保证了这个对象的唯一性
   * */
  ScopeManager::GetInstance().ClearScope();
  //  递归的去寻找当前obj中的函数，类，方法等，并为之格式化命名
  //  先清空当前的作用域名称
  /*以Lenet5网络为例:
   *cellscope_name:Default/optimizer-Momentum
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5
    cellscope_name:Default/network-WithLossCell/_loss_fn-SoftmaxCrossEntropyWithLogits
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5/conv1-Conv2d
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5/conv2-Conv2d
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5/fc1-Dense
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5/fc2-Dense
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5/fc3-Dense
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5/relu-ReLU
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5/sigmoid-Sigmoid
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5/max_pool2d-MaxPool2d
    cellscope_name:Default/network-WithLossCell/_backbone-LeNet5/flatten-Flatten
   * 为Cell中的小的Cell设置对应的作用域
   * */
...
FuncGraphPtr Parser::ParseFuncGraph() {
  ...
  /*2020年8月29日 11:37:36
   * 获取抽象语法树的节点，其实就是根节点，一个方法或者一个类
   * 并且将获得的节点转化成函数块
   * 这里的对象是parsePythoncode中new出来的parse对象
   * */
...
void Parser::GenerateArgsDefaultValueForFunction(const FunctionBlockPtr &block, const py::object &fn_node) {
  ...
  for (std::size_t i = 0; i < args.size(); i++) {
    ...
    // 如果默认值的none 那么根据ms的解析创建一个kNull为宏的值
    // 否则解析表达式并且创建一个默认参数节点
...
FunctionBlockPtr Parser::ParseFunction(const py::object &node, const FunctionBlockPtr &block) {
  /* 获取当前的命名空间，使用当前对象构造Function_block
   *
   *
   * */
  ScopePtr scope = GetScopeForParseFunction();
  // the node created in the parse function context, will inherit the scope created using scope_guard
  // 将当前的scope放入容器中
  ScopeGuard scope_guard(scope);
  //data_converter::GetObjKey(ast()->obj())[0] 获取 obj的id
  //GetLocation(node)  获取节点的位置
  TraceGuard trace_guard(data_converter::GetObjKey(ast()->obj())[0], GetLocation(node));
  /*对当前的Parser生成新的FunctiofnBlock
   * */
//  11：23 这个不理解
  /* 2020-09-17 22:24:22 ↓
   * 返回的是 std::make_shared<FunctionBlock>(parse);
   * 将当前的parse抓化成functionblock，先将上下文转移到pFunBlock中再进行转化
   * */
  FunctionBlockPtr pFunBlock = MakeFunctionBlock(*this);
  /* 如果block == nullptr 那么就只有一个图
   * 否则就是嵌套图，需要为当前的图添加一个前驱图
   * 当前图的输出作为前驱图的一个输入
   * */
  if (block != nullptr) {
 ...
 *  @Return 解析完毕后返回处理好的函数块
 *  调用节点的 expand_expr_statement 方法判断该节点的扩展信息
 *   True, expr.value, x
 *   True, expr.value
 *   False, None, None
 *  Ture 代表不为空
...
/*2020-09-16 22:26:39 ↓
 * 将ast上的数值转化成一个ValueNode节点
 * 值只有两种类型一种是int，另一种是float
 * 返回对应的ValueNode节点
 * @param node 类型为Num的ast上的节点
 * @FunctionBLock 这里似乎没有用
 * */
...
/* 2020-09-16 22:59:16 ↓
 * @param node ast树上的节点，属性应该是attr
 * @return ValueNode value为String
 * */
 ...
 * @params: block 当前解析的函数图
 * @params: node Attribute的节点类型
 * @return: node解析成的值并且转化成AnfNode节点
 * @date: 2020-9-22
 * @author: geray
 * @Function： 解析属性值
 *
...
AnfNodePtr Parser::ParseIfExp(const FunctionBlockPtr &block, const py::object &node) {
  ...
  // process the if-true branch
  py::object bodyNode = python_adapter::GetPyObjAttr(node, "body");
  true_block->func_graph()->debug_info()->set_location(



    GetLocation(bodyNode));
...
/* 9-16 12：14
 * 当Assign的左边是一个简单变量的时候
 * 执行此函数
 * 取得包装后的id加上命名空间，然后放入缓冲区中
 * @block 当前处理的Functionblock
 * @targ  Assign等式左边的变量
 * @assigned_node  解析后的值
 * @return void
 * */
...
/*该函数将target进行分类，分成三种类型交给不同的函数管理
 * */
...
/* 将赋值语句转化成计算图
 * 从抽象语法树上取得'='后面的值，然后再解析
 * 将解析的结果赋值给'='前面的变量，然后根据 变量的个数进行写值，即有几个值分别写入block中
 * */
 ...
 * 根据不同的类型来调用python端的parse函数将Function转化成AST树
 * 并且给AST树添加基础的信息
 * function_module : 该函数所在的文件
 * function_name : 该函数的名称
 * filename : 该函数所在文件的绝对路径
 * line_offset : 该函数所在文件的行偏移量(即在第几行)
 * 结束后当前类的ast_tree_就处理完毕了
 * */
...
//  如果是方法
  ...
  /*如果是类中的方法，获取方法的名字
   * 并且将方法函数对象赋给obj_(// save obj,eg: class instance or function
   * */
  ...
  /*如果是一个类，那么就获取这个类的construct方法或是hook_contruct方法，并将这个函数对象赋值给function_
   * */
  ...
  /*2020年8月25日 20:45:39 👇
   *以上做的实际上是为了统一化管理，最后需要获取的之后函数或者方法名
   * 1. 用户使用函数定义网络
   * 2. 用户使用继承nn.Cell的类定义网络
   * 3. 用户使用一个类中的方法定义网络
   * 处理上述三种情况之后，就可以利用PVM的反射来获取用户的代码，并生成AST
   * 初始化python端的Parser.py
   * */
  /*这句话执行用第三第四个参数作为参数来执行PYTHON_MOD_PARSE_OBJECT_FUNCTION函数
   * 执行python端的Parse构造函数
   *
   * */
...
// 获取整个函数的输入参数类型，如:训练时输入的是data和label
