...
// ConvertToFuncGraph 将改函数对象转化成ANF图
...
/*参数:
 * ResourcePtr &res : 带有网络信息以及管理器的智能指针
 * vector<PassItem>&passes: 是一个vector<vector<string,function>>,其中的函数信息包含如下
 *
 * */
...
#endif
/*
 * 并行基元相关的valuenode可能会被分区，以便其值因设备而异，
 * 这将由于执行顺序不同而导致同步错误。
 * 在这里，我们通过跳过并行相关基元使用的valuenode合并暂时避免了该问题，
 * 最终解决方案是 后来作为并行功能提出。
 * */
