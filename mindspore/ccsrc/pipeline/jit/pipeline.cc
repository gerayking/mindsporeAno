...
/* resource 是py端传下来的对象的智能指针
 * phase_s 是py的str类型强制转化成C++的string类ing
 * use_vm : 猜测是一个标注是否开启某种模式
 * 2020年8月17日16点20分
 * 此函数的作用是获取一些方法指针
 * */
std::vector<ActionItem> GetPipline(const ResourcePtr &resource, const std::string &phase_s, bool use_vm) {
  ...
  /*Mscontext 是一些底层的配置类，应该是单例模式，可以从中获取默认的设置
   * 例如:使用CPU或者GPU或者Ascend等等
   * 这里应该是获得编译的策略
   * 2020年8月17日16点31分
   * */
...
/*参数:
 * py::object &obj : 传下来的网络模型,继承了nn.cell
 * py::tuple  &args : 传下来的数据
 * py::pyject &phase : 模式
 * bool use_vm : 是否启动自动并行
 * */
...
/*
 * 用一个智能指针存储py端传送下来的训练网络结构以及需要优化的方法
 * FilterActions 返回类型为: vector<ActionItem> 即
 * Vector<pair<string,function<bool(ResourcePtr)
 *
 * */
  // get the parameters items and add the value to args_spec
  /*获取训练集中的参数并把它封装到args_spec中*/
  abstract::AbstractBasePtrList args_spec;
  std::size_t size = args.size();
  for (std::size_t i = 0; i < size; i++) {
//    初始化一个指向数据的智能指针
    ValuePtr converted = nullptr;
//    此处是将python的数据格式转化为MS中C++端自定义的数据格式
...
#endif
/* 2020年8月20日 23:36:04
 * 把遍历方法函数并对每一个进行操作，如果ENABLE_LOAD_ANF_IF即生成ANF图，那么就送入RunPipeAction中生成相应的日志图形
 * 否则就将resource的引用送入该函数中进行优化
 *
 * */
void Pipeline::Run() {
  /*运行Pinpeline的流程
